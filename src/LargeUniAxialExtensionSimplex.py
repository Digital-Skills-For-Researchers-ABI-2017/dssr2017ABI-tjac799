...
if(UsePressureBasis):
    ...
    pressureBasis.interpolationXi = [iron.BasisInterpolationSpecifications.LINEAR_SIMPLEX]*numberOfXi
    if(NumberOfGaussXi>0):
        pressureBasis.quadratureOrder = 2
...
if exportFields:
    ...
    fields.NodesExport("LargeUniaxialExtensionSimplex","FORTRAN")
    fields.ElementsExport("LargeUniaxialExtensionSimplex","FORTRAN")
    fields.Finalise()
    exit()

print 'not exporting mesh for visualisation, continueing with analyis'
...
#Set x=0 nodes to no x displacment in x. Set x=width nodes to 10% x displacement, checked by Toby 15/6/17
...
fields = iron.Fields()
fields.CreateRegion(region)
fields.NodesExport("SimplexLargeUniaxialExtension","FORTRAN")
fields.ElementsExport("SimplexLargeUniaxialExtension","FORTRAN")
fields.Finalise()
